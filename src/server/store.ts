...
function escalateOverdueSignals(state:DemoState){
  let changed=false;
  state.signals=state.signals.map(signal=>{
    if(signal.status==="collected"||signal.dispatchReviewAt||Date.parse(state.now)-Date.parse(signal.createdAt)<4*60*60*1000)return signal;
    changed=true;
    append(state,"signal.sla_escalated",signal.id,"Four-hour Waste Ready SLA reached; dispatcher review created and priority raised.");
    return {...signal,dispatchReviewAt:state.now};
  });
  if(changed)state.route=replanMidTrip(state,"waste_ready_sla_escalation");
}
...
function serviceCurrentStop(state:DemoState){
  ...
  if(!stop) return;
  // A collector may complete this stop manually while the autonomous demo
  // truck is still approaching it. Advance without creating a second proof.
  if(stop.status==="collected"||stop.status==="blocked")return;
  ...
  if(stop.kind==="bin"){
    ...
    state.signals=state.signals.map(s=>s.id===stop.workId?{...s,status:"collected",proofStatus:"pending",citizenOutcome:undefined}:s);
    state.proofs.unshift({id:`proof-engine-${state.proofs.length+1}`,reportId:stop.workId,stopId:stop.id,capturedAt:state.now,status:"pending_sync",note:"Collection recorded; collector evidence is required before citizen closure.",source:SYNTHETIC_SOURCE});
    engineEvent(state,"signal.collected",stop.workId,`Citizen pickup at ${stop.label} completed; cleanup proof is pending.`);
  } else {
...
export function tick(seconds=30){const state=structuredClone(getState());state.now=plusSeconds(state.now,seconds);append(state,"demo.ticked","scenario-mahadevapura",`Simulation advanced ${seconds} seconds.`);escalateOverdueSignals(state);return setState(state)}
export function createSignal(input:{clientSignalId?:string;type:"have_waste"|"waste_outside";category:string;amountBand:"small"|"medium"|"large";location:{lat:number;lng:number}}){const state=structuredClone(getState());const id=input.clientSignalId??`sig-api-${state.signals.length+1}`;if(state.signals.some(signal=>signal.id===id))throw new Error("DUPLICATE_SIGNAL_ID");const signal:WasteSignal={id,type:input.type,category:input.category,amountBand:input.amountBand,locality:"Whitefield",location:input.location,status:"queued",createdAt:state.now,etaMinutes:9,source:SYNTHETIC_SOURCE};state.signals.unshift(signal);append(state,"signal.created",signal.id,"Citizen signal received.");state.route=replanMidTrip(state,"new_citizen_signal");append(state,"route.revised",state.route.id,"Route recomputed after citizen demand.");setState(state);return {signal,etaMinutes:9,route:state.route}}
...
export function confirmSignal(signalId:string,outcome:"cleaned"|"partial"|"still_present"){const state=structuredClone(getState());const signal=state.signals.find(item=>item.id===signalId);if(!signal)throw new Error("NOT_FOUND");if(signal.proofStatus!=="accepted")throw new Error("INVALID_STATUS_TRANSITION");signal.citizenOutcome=outcome==="cleaned"?"confirmed":"reopened";if(outcome!=="cleaned"){signal.status="queued";state.route=replanMidTrip(state,"citizen_reopened_signal")}append(state,outcome==="cleaned"?"signal.confirmed":"signal.reopened",signalId,outcome==="cleaned"?"Citizen confirmed pickup completion.":"Citizen reopened the pickup because waste remains.");setState(state);return signal}
